...
def get_mood_stress_30_days(user_id: str):
    start = (date.today() - timedelta(days=30)).isoformat()
    
    response = (
        supabase.table("StressLog")
        .select("*")
        .gte("date", start)
        .order("date", desc = False)
        .eq("userID", user_id)
...
def get_30_days_data(user_id: str):
    ...
    stress = get_mood_stress_30_days(user_id)
...
def test():
    ...
    print("STRESS COLUMNS:", data["stress"][0].keys() if data["stress"] else "empty")
    ...
    print("\STRESS ROW:", json.dumps(data["stress"][0], indent=2) if data["stress"] else "empty")
